...
// this query will get all the podcasts based on the voiceType of the podcast , which we are showing in the Similar Posts section.
...
export const getAllPodcasts = query({
  handler: async (ctx) => {
    return await ctx.db.query("podcasts").order("asc").collect();
  },
});
...
export const getTrendingPodcasts = query({
  handler: async (ctx) => {
    ...
    return podcast.sort((a, b) => a.views - b.views).slice(0, 100);
  },
});
